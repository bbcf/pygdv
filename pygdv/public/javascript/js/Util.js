...
/**
 * Commas look too much like periods which are by convention
 * used for the decimal point. Quotes are hence much
 * better than commas for secondary number seperators.
 */
Util.addSeperator = function(nStr, seperator) {
    ...
    x2 = x.length > 1 ? '.' + x[1] : '';
    var rgx = /(\d+)(\d{3})/;
    while (rgx.test(x1)) {x1 = x1.replace(rgx, '$1' + seperator + '$2');}
    return x1 + x2;
}
