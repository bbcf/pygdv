...
class TrackController(CrudRestController):
    ...
    def post(self, *args, **kw):
        ...
        del kw['sprox_id']
        '''
        {'file_upload': FieldStorage('file_upload', u'toto.txt'), 
        'sprox_id': None, 
        'urls': u'dcdc\r\nggg\r\njkl,jhg'}
...
#            file_value = kw['file_upload'].value
            #files.append(handler.util.upload(file_upload=kw['file_upload']))
        if 'urls' in kw:
            print 'get urls %s' %(kw['urls'])
            for u in kw['urls']:
                print u
...
#        # get a tmp directory
#        tmp = handler.util.get_unique_tmp_directory()
#        # write the file in
#        file_path = os.path.join(tmp,filename)
#        f = open(file_path, "w")
#        f.write(file_value)
